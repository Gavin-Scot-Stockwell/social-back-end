...
export const updateThought = async (req, res) => {
    try {
        ...
        const updatedThought = await Thoughts.findOneAndUpdate({ _id: thoughtId }, updateData, { new: true });
        if (!updatedThought) {
...
export const createReaction = async (req, res) => {
    try {
        const thought = await Thoughts.findOneAndUpdate({ _id: req.params.thoughtId }, { $addToSet: { reactions: req.body } }, { new: true });
        if (!thought) {
            return res.status(404).json({ message: 'No thought with this ID' });
        }
        return res.json(thought);
...
export const deleteReaction = async (req, res) => {
    try {
        const thought = await Thoughts.findOneAndUpdate({ _id: req.params.thoughtId }, { $pull: { reactions: { _id: req.params.reactionId } } }, { new: true });
        if (!thought) {
            return res.status(404).json({ message: 'No thought with this ID' });
        }
        return res.json(thought);
